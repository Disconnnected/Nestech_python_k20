...
def calculate_cost(usage, prices):
    ...
    usage_int = int(usage)
    if(usage_int <= 50):
        result = usage_int * prices["first_50"]
    elif((usage_int > 50) & (usage_int <=100)):
        result = 50*prices["first_50"] + (usage_int-50)*prices["from_51_to_100"]
    else:
        result = 50*prices["first_50"] + 50*prices["from_51_to_100"] + (usage_int-100)*prices["above_100"]
    result = f'{result:,}'
    return result
    # pass
